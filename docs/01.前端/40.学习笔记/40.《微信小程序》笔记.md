...
* app.json [全局配置API](https://developers.weixin.qq.com/miniprogram/dev/reference/configuration/app.html)
...
尺寸单位：rpx，根据屏幕宽度自适应。375px*750px下，1px=2rpx
...
```javascript
<button bindtap="onTapHandler" data-hello="hello world">点我+1</button>
<view>{{count}}</view>
```
...
> 阻止事件冒泡：把绑定方式 bindtap 换成 catchtap 即可。
...
微信客户端天然鉴权，直接获取appid、获取openid、生成分享图、调用腾讯云SDK ...



##### 云数据库

云开发提供了一个 JSON 数据库，完成数据的增、删、改、查 ...
...
轻松实现管理文件、上传文件、下载文件、分享文件 ...

> **每个小程序账号可免费创建两个环境，建议：开发环境、生产环境**
#### 云数据库能力
...
> serverless（无服务） 概念：函数即服务，当需要后端服务的时候，不需要关心后端的IP地址、域名，只需要像调用普通函数一样既可以实现调用。
...
key的值不建议使用index，因为当数据发生变化时，会重新渲染dom结构，使用index的地方不会随之变化(参考Vue.js中v-for的使用)。
...
云函数目录  `httpDemo` 右键 `在终端打开` 打开命令行  输入命令：
...
const URL = 'http://musicapi.xiecheng.live/personalized'// 服务器接口地址

// 云函数入口函数
exports.main = async (event, context) => {
  const playlist = await rp(URL).then((res) => {
    // 请求自己服务器接口返回的数据是字符串形式的json对象，需要序列化
