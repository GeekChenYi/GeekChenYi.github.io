...
`mapState` 函数返回的是一个对象。可使用对象展开运算符将此对象混入到外部对象中，**这种方法也是最常用的一种方法。**
...
默认情况下，模块内部的action，mutations和getters是注册在全局命名空间的，这样使得多个模块能够对同一mutations或actions作出响应。

如果希望你的模块具有更高的封装度和复用性，可以通过添加namespace:true的方式使其成为带命名空间的模块。
...
const mutations = {
  ...
  SET_INTRODUCTION: (state, info) => {
    state.introduction = info
  },
  // 这种写法也是可以的
  SET_NAME(state, name) {
    state.name = name
...
const actions = {
  getInfo({commit, state}){
    ...
    commit('SET_NAME', userInfo.avatar)
    commit('SET_NAME', userInfo.introduction)
  }
}
