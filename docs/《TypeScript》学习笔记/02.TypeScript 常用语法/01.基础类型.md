...
::: warning 
void操作符后边跟任何东西都是返回undefined，又因为 void 0这个表达式最短，因此一般都会写成：void 0
:::
...
unknown类型在很多方面不同于any类型，如果不缩小范围，就无法对unknown类型执行任何操作。
...
一种更安全的缩小未知类型的方法是使用 类型收缩 。TypeScript 编译器会分析我们的代码，并找出一个更窄的类型。
